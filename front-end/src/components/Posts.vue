        ...
        <v-card  class="main_post" v-for="(post,index) in this.$store.state.posts" v-bind:key="index">  
            <v-container class="post--container">
                ...
                <v-card-actions  v-if="this.$store.state.user.admin == true || post.userId == this.$store.state.user.userId" class="btn--update">
                    ...
                    <v-btn class="--button" @click="openConfirmDelete(post.id)" >Supprimer l'article</v-btn>
                </v-card-actions>
...
export default {
    ...
    methods: {
        ...
        .then( () =>{
            this.refreshPost()
            this.$store.commit('SETSTATUS' , {status:'success',message:`Votre Post a bien etait suprimer`}); //type et payload
        })
        .catch(error => {
            this.$store.commit('SETSTATUS' , {status:'error',message:`Impossible de supprimer le post ${error}`}); //type et payload
        })
        },
